...
def main_page():
    ...
    with st.sidebar:
        ...
        age = st.number_input("Age of Patient", min_value=2, max_value=10, value=9, key=f"age_{reset_key}", help="Patient age between 2 to 10 years")
        va_left = st.number_input("Visual Acuity (Left Eye)", min_value=0.0, max_value=1.0, value=0.74, step=0.05, key=f"va_left_{reset_key}", help="1.0 = Perfect vision, 0.0 = No vision")
        va_right = st.number_input("Visual Acuity (Right Eye)", min_value=0.0, max_value=1.0, value=0.49, step=0.05, key=f"va_right_{reset_key}", help="1.0 = Perfect vision, 0.0 = No vision")
        refractive_error = st.number_input("Refractive Error", min_value=0.0, max_value=5.0, value=1.82, step=0.1, key=f"refractive_{reset_key}", help="Higher value means more refractive error")
        ocular_alignment = st.selectbox("Ocular Alignment", ["Normal", "Misaligned"], index=1, key=f"ocular_{reset_key}")
        vision_result = st.selectbox("Vision Screening Result", ["Pass", "Refer", "Fail"], index=2, key=f"vision_{reset_key}")
        strabismus = st.selectbox("Strabismus Present?", ["No", "Yes"], key=f"strabismus_{reset_key}")
        family_history = st.selectbox("Family History?", ["No", "Yes"], index=1, key=f"family_{reset_key}")
        premature_birth = st.selectbox("Premature Birth?", ["No", "Yes"], key=f"premature_{reset_key}")
        eye_patch = st.selectbox("Eye Patching Treatment?", ["No", "Yes"], index=1, key=f"patch_{reset_key}")
